...
class SimpleMicroscopeController(QObject):
    ...
    def __init__(self, epics_pv=DEFAULT_SCAN_PV, status_pv=DEFAULT_STATUS_PV, fine_scan_params_path="fine_scan_params.json", controls_json_path=None):
        ...
        self.queue_manager.zoom_scan_completed.connect(self.on_zoom_scan_completed)
    ...
    def wait_for_zoom_completion(self):
        ...
        self._poll_zoom_scan_completion()

    def _poll_zoom_scan_completion(self):
        from bluesky_queueserver_api import REManagerAPI
        RM = REManagerAPI()
        if self.epics_manager.wait_for_scan_completion():
            self.queue_manager.zoom_scan_completed.emit()
        else:
            print(f"[MAIN] Zoom scans failed or timed out")
            self.complete_loop()

    def on_zoom_scan_completed(self):
        from bluesky_queueserver_api import REManagerAPI
        RM = REManagerAPI()
        ...
        if zoom_plans:
            ...
            QTimer.singleShot(1000, self._poll_zoom_scan_completion)
        else:
...
def main():
    ...
    print(f"[MAIN] EPICS scan status PV: {args.epics_pv}")
